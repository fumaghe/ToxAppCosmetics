...
from app.utils.web_utils import get_pubchem_cid, get_ld50_pubchem, extract_first_status_link
from app.utils.value_extraction_utils import find_values, display_values, find_most_common_values
from app.utils.web_utils import find_missing_values
...
def search_ingredient(ingredient_name_or_id):
    ...
    cursor = conn.cursor()
    query = """
    SELECT pcpc_ingredientid AS id, pcpc_ingredientname AS name, NOAEL_CIR, LD50_CIR, LD50_PubChem, value_updated
    FROM ingredients
...
def find_ingredient_id_and_extract_link(ingredient_name):
    ...
    if ingredient:
        ...
        # Esegui la ricerca online se l'ingrediente non è nel database
