...
def search_ingredients(start_index, end_index, stop_flag):
    ...
    progress_bar = tqdm(ingredients, desc="Processing ingredients")
    for ingredient_id, ingredient_name in progress_bar:
        if stop_flag:
            break
        progress_bar.set_description(f"Processing {ingredient_name}")
        try:
            ...
            conn.commit()
            logging.info(f"Updated database for ingredient {ingredient_name}: CIR Page - {cir_page}, CIR PDF - {cir_pdf}, PubChem Page - {pubchem_page}")

            # Verifica immediatamente dopo l'aggiornamento
