...
def process_ingredients(selected_ingredients):
    terms = ["NOAEL", "LD50"]
    ...
    for ingredient_name in selected_ingredients:
        ...
        cursor.execute("SELECT pcpc_ingredientid FROM ingredients WHERE pcpc_ingredientname=?", (ingredient_name,))
        result = cursor.fetchone()
        if result:
            pcpc_ingredientid = result[0]
        ...
        print(f"Results for {ingredient_name}:")
        if "NOAEL" in results and results["NOAEL"]:
            print("NOAEL values:")
            for value, context in results["NOAEL"]:
        ...
        if "LD50" in results and results["LD50"]:
            print("LD50 values:")
            for value, context in results["LD50"]:
                print(f"Value: {value} Context: {context}")
        else:
            print("No LD50 values found.")

    conn.close()

# Elenco degli ingredienti da testare
test_ingredients = [
...
# Processa gli ingredienti di test
