...
def find_noael_values(text):
  ...
  pattern = r'NOAEL\s*[:/]'  # Match NOAEL followed by whitespace, colon or slash
  matches = re.finditer(pattern, text, re.IGNORECASE)
  values = []
  for match in matches:
    # Extract digits after NOAEL (up to 100 characters)
    digits = re.findall(r'\d+(?:\.\d+)?', text[match.end():match.end()+100])
    if digits:
