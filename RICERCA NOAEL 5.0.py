...
def check_and_append_noael(ingredient_id, noael_values, noael_file_path):
    with open(noael_file_path, 'a', encoding='utf-8') as file:
        file.write(f"{ingredient_id}:{','.join(map(str, noael_values))}\n")
        ...
        for line in file:
            if ':' in line:
                ...
                if len(id_value_pair) == 2:
                    noael_dict[id_value_pair[0]] = list(map(int, id_value_pair[1].split(',')))
    ...
    if ingredients:
        for ingredient_name, ingredient_id in set(ingredients):
            if ingredient_id in noael_dict:
                noael_values = noael_dict[ingredient_id]
                ...
                if status_link:
                    try:
                        ...
                        if noael_values:
                            ...
                            check_and_append_noael(ingredient_id, list(set(noael_values)), noael_file_path)
                        else:
                            print(f"Ingredient name: {ingredient_name}")
